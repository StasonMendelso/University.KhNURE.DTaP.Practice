 ...
 */
public class OutcomeRecordLoggerDaoObserver extends OutcomeRecordDaoObserver {
    private static final Logger LOGGER = Logger.getLogger(OutcomeRecordLoggerDaoObserver.class.getName());
    private final Map<DaoEventType, Function<RecordDaoEvent, String>> messageMap;

    public OutcomeRecordLoggerDaoObserver(List<DaoEventType> daoEventTypeList) {
        super(daoEventTypeList);
        this.messageMap = new EnumMap<>(DaoEventType.class);
        initMap();
    }

    public OutcomeRecordLoggerDaoObserver() {
    ...
    private static String createLogMessage(RecordDaoEvent recordDaoEvent, String singleMessage, String singleDetailsMessage,
        ...
        if (recordList.size() == 1) {
            return String.format("Сутність Record(OutcomeJournal) з id = %d %s. %s: %s", recordList.get(0).getId(), singleMessage, singleDetailsMessage, recordList.get(0).toString());
        }
        StringBuilder builder = new StringBuilder(new String("Сутності Record(OutcomeJournal) "));
